# 최종 제출 메모

- 서비스명: 오늘 어디로? - 동네 단위 AI 여행·지역생활 플래너
- Vercel URL: `여기에 실제 주소 입력`
- GitHub URL: `여기에 실제 주소 입력`
- README: GitHub 저장소의 `README.md`
- 서비스 기획서: `docs/SERVICE_PLAN.md`

## 증빙 파일

- 데스크톱: `evidence/01-desktop-home.png`
- 모바일: `evidence/02-mobile-responsive.png`
- AI 입력·결과: `evidence/03-ai-input-result.png`
- 실패 처리: `evidence/04-error-message.png`
- 보너스 기능: `evidence/05-bonus-dark-history.png`
- AI 코딩 도구 과정: `evidence/06-ai-coding-process.png`

## 한 문장 설명

사용자가 현재 위치 또는 동 단위 지역과 여행 조건을 입력하면, 실제 API로 확인된 장소 안에서 AI가 당일 일정과 음식점·카페·주차장을 추천하는 반응형 웹서비스입니다.

